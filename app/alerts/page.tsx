...
import { ShieldAlert, Check, Trash2, Mail, Phone, Settings, Database } from "lucide-react";
import { AlertsFeed } from "@/components/alerts/AlertsFeed";
import { isSupabaseConfigured } from "@/lib/supabase";

export default function AlertsPage() {
  const { alerts, acknowledgeAlert, dismissAlert } = useGroundwaterStore();
  const {
    emailNotifications,
    smsAlerts,
    criticalOnly,
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-neutral-divider">
        <div>
          ...
          <p className="text-xs text-neutral-secondary">
            {supabaseEnabled 
              ? "Real-time Supabase alerts with live updates and threshold monitoring"
              : "Hazard notifications, critical depletion thresholds, and notification controls"
            }
          </p>
        </div>
      </div>

      {/* Show Supabase Real-Time Feed if configured */}
      {supabaseEnabled ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2">
            <AlertsFeed limit={50} showFilters={true} />
          </div>
            ...
            <div className="space-y-4 pt-2 border-t border-neutral-divider">
              ...
              <div className="pt-4 border-t border-neutral-divider">
                <div className="flex items-center gap-2 text-xs text-emerald-600 font-medium">
                  ...
                  <span>Connected to Supabase Real-Time</span>
                </div>
              </div>
            </div>
          </Card>
        ...
        /* Fallback to mock alerts if Supabase not configured */
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Left Column: Mock Alert History Log */}
